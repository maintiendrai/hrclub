...
package com.hrclub.model.upload ;
...
public class SmartUpload
{
    ...
    public final void initialize(ServletContext ctx, HttpServletRequest request, HttpServletResponse response)
        throws ServletException
    {
        m_application = ctx;
        m_request = request;
        ...
        do
        {
            ...
            if(isFile && fileName.length() > 0)
            {
                if(m_deniedFilesList.contains(fileExt))
                    throw new SecurityException("文件类型不合法！必须是JPG或GIF文件！");
                ...
                if(m_maxFileSize > (long)0 && (long)((m_endData - m_startData) + 1) > m_maxFileSize)
                    throw new SecurityException(String.valueOf((new StringBuffer("文件大小超出范围：")).append(fileName)));
            ...
            if(isFile)
            {
                com.hrclub.model.upload.File newFile = new  com.hrclub.model.upload.File();
