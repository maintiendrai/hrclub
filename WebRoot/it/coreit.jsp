...
%>
			...
			<tr>
				<td class="small"><a href='<%=request.getContextPath()%>/it/itbaseinfo.do?operation=itbaseinfo&sign=hrsign&userid=<%=u.getUserid()%>'><%=u.getName()%></a></td>
				<td class="small">
				...
				<td class="small"><a href='<%=request.getContextPath()%>/it/itbaseinfo.do?operation=itbaseinfo&sign=hrsign&userid=<%=u.getUserid()%>'>查看</a>
				</td>
			</tr>
			<tr>
			<td colspan="4">
...
<tr>
	...
	%>
	<a href='showhr.do?operation=coreit&pageNum=1&pageLength=<%=mypage.getPageLength() %>'>首页</a>&nbsp;&nbsp;
	<a href='showhr.do?operation=coreit&pageNum=<%=mypage.getPageNum()-1%>&pageLength=<%=mypage.getPageLength()%>'>上页</a>&nbsp;&nbsp;
	<%
	}%>
	...
	%>
	<a href='showhr.do?operation=coreit&pageNum=<%=mypage.getPageNum()+1%>&pageLength=<%=mypage.getPageLength()%>'>下页</a>&nbsp;&nbsp;
	<a href='showhr.do?operation=coreit&pageNum=<%=mypage.getPageCount()%>&pageLength=<%=mypage.getPageLength()%>'>末页</a>
	<%
	}}%> 
